...
def heatmap_monomer_count(FileName: str, FileNum: int, InitialTime: float, FinalTime: float,
    ...
    for histogram_file_number in range(1, FileNum+1):
        ...
        #determines number of monomers in each complex size
        z_plt_mod = []
        for complex_size,protein_complex in enumerate(z_plt):
            z_plt_mod_temp = []
            for time_bin in protein_complex:
                z_plt_mod_temp.append(time_bin * complex_size)
        ...
        for time_bin in z_plt:
            ...
            for index,protein_complex in enumerate(time_bin):
                x_count += 1
                sum_ += protein_complex
    ...
    #ensure that the average count of monomers in a certain species size list has equal length to the other ... lists
    for file in z_list_tot:
        for time_bin in file:
            if len(time_bin) < len(n_list):
                for na in range(0, 1 + len(n_list) - len(z_list_tot[i][j])):
                    time_bin.append(0.0)
